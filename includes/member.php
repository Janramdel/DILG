...
class User{
	...
	protected static $tbl_name = "tbl_useraccount";
	function db_fields(){
		global $mydb;
	...
	function find_all_user($user=""){
			global $mydb;
			$mydb->setQuery("SELECT * 
							FROM  ".self::$tbl_name." 
							WHERE `ACCOUNT_USER` = '{$user}'");
			$cur = $mydb->executeQuery();
			$row_count = $mydb->num_rows($cur);//get the number of count
			return $row_count;
	}

	 static function AuthenticateUser($h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tbl_useraccount` WHERE `ACCOUNT_USERNAME`='" . $email . "' and `ACCOUNT_PASSWORD`='" . $h_upass ."' LIMIT 1");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
		 $found_user = $mydb->loadSingleResult();
		    $_SESSION['ACCOUNT_USER'] 	 = $found_user->ACCOUNT_USER;
	...
	function single_user($id=0){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where ACCOUNT_USER= {$id} LIMIT 1");
